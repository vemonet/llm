...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            FunctionBuilder::new("get_weather")
                ...
                .param(
                    ParamBuilder::new("time_period")
                        .type_of("string")
                        .description(
                            "When you want weather info for, e.g., 'May', 'next week', 'tomorrow'",
                        ),
                )
    ...
    if let Some(tool_calls) = tool_calls {
        ...
        for tool_call in &tool_calls {
            ...
            // Serialize the result to a string for the arguments
        ...
        let final_response = llm
            .chat_with_tools(&follow_up_messages, llm.tools())
            .await?;
        println!(
    ...
    let weather_messages = vec![ChatMessage::user()
    ...
    let forced_response = forced_llm
        .chat_with_tools(&weather_messages, forced_llm.tools())
        .await?;
